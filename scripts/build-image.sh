...
. /usr/share/revs-docker-internal/ubuntu-base/library.sh
...
} | tee /etc/dpkg/dpkg.cfg.d/100-revstech-dpkg > /dev/null
...
# be permissive with packages without visa.
{
    ...
    echo 'Dpkg::Post-Invoke { "/usr/share/revs-docker-internal/ubuntu-base/clean-image.sh"; }; '
} | tee /etc/apt/apt.conf.d/100-revstech-apt > /dev/null
...
cat >> /etc/postgresql-common/common-revstech.conf << EOF
listen_addresses = '*'
temp_buffers = 16MB
work_mem = 16MB
